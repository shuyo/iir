...
class DP_MRM:
    ...
    def insert_table(self, j, v):
        ...
        self.kl_jt[j][t_new] = self.sampling_kl_for_new_table(j, v)

    def sampling_kl_for_new_table(self, j, v):
        kl_list = [(k, l) for l in self.using_l[k] for k in self.labels[j]]
        p_kl = [self.m_kl[k][l] * self.n_klv[k][l][v] / self.n_kl[k][l] if l > 0 else self.gamma / self.V for (k, l) in kl_list]
        k_new, l_new = kl_list[numpy.random.multinomial(1, p_kl / p_kl.sum()).argmax()]
        if l_new == 0:
            l_new = self.insert_dish(k)
        return k_new, l_new
