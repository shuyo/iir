...
class PoissonIRM(object):
    ...
    def update_cluster(self, k, s1, s2, n1, n2, R):
        ...
        c1 = len(n1)
        c2 = len(n2)
        m1, m0, m1k, m0k = sum_nij(R, s1, s2, c1, c2)

        logps = numpy.zeros(c1+1)
        for i in range(c1):
            p = numpy.log(n1[i])
            p += gln(self.a+m1[i]+m1k, self.b+m0[i]+m0k).sum()
            p -= gln(self.a+m1[i], self.b+m0[i]).sum()
            logps[i] = p
        p = numpy.log(self.alpha)
        p += betaln(self.a+m1k, self.b+m0k).sum()
        p -= c2 * betaln(self.a, self.b)
    ...
    def log_posterior(self):
        log_v = log_ps(self.alpha, self.n1, self.K)
        log_v += log_ps(self.alpha, self.n2, self.L)
        m1, m0, m1k, m0k = sum_nij(self.R, self.s1, self.s2, len(self.n1), len(self.n2))
        beta_ab = gln(self.a, self.b)
        for m1i, m0i in zip(m1, m0):
            for n1, n0 in zip(m1i, m0i):
                log_v += gln(n1 + self.a, n0 + self.b) - beta_ab
        return log_v
